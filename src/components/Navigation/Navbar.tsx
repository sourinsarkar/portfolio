    ...
    return (
        <nav className="inline-block fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50">
            <ul className="flex items-center NavList space-x-2">
                <li className={`${isActive('/home') ? 'shadow-is1' : ''}`}><Link href="/home"><RiHome4Line size={iconSize} /></Link></li>
                <li><a target="_blank" href="https://dribbble.com/sourinsarkar"><RiBrushLine size={iconSize} /></a></li>
                <li className={`${isActive('/projects') ? 'shadow-is1' : ''}`}><Link href="/projects"><RiLightbulbFlashLine size={iconSize} /></Link></li>
